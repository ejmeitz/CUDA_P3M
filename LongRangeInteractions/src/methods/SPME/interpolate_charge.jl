...
function interpolate_charge2!(u, Q, dQdr, spme::SPME{SingleThread})
    ...
    M0, M1, M2, dM0, dM1, dM2 = calc_spline_values(u, n, N_atoms)
    v = 0.0
    l0 = 0; l1 = 0; l2 = 0;
    n_half = ceil(Int64,n/2)
    u_i = [0,0,0]
    for i in 1:N_atoms
        v = charges(spme.sys,i)
        u_i .= round.(Int64,u[i])
        for c0 in 0:n
            l0 = u_i[1] - c0 # Grid point to interpolate onto
            ...
            for c1 in 0:n
                l1 = u_i[2] - c1 # Grid point to interpolate onto
                ...
                for c2 in 0:n
                    l2 = u_i[3] - c2 # Grid point to interpolate onto
